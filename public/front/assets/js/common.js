...
$(document).on('click', '.qty_increase', function () {
    ...
    $.ajax({
        method: "POST",
        url: $('#CartIncrementFromSession').data("url"), // অবশ্যই route /cart/increase হওয়া উচিত
        data: {
            _token: $('meta[name="csrf-token"]').attr('content'),
            id: id,
            quantity: quantity2
                ...
                <div class="product-item cart-product-item">
                    <div class="single-grid-product">
                        <div class="product-top">
                            <a href="#"><img class="product-thumbnal" src="${Img}/${obj[key]['options']['image']}" alt="cart"></a>
                        </div>
                        <div class="product-info">
                            <div class="product-name-part">
                                <h3 class="product-name"><a class="product-link" href="#">${obj[key]['name']}</a></h3>
                                <div class="cart-quantity input-group">
                                    <div class="increase-btn dec qtybutton btn qty_decrease" data-id="${obj[key]['rowId']}">-</div>
                                    <input class="qty-input cart-plus-minus-box qty_value" type="text" name="qtybutton" value="${obj[key]['qty']}" readonly />
                                    <div class="increase-btn inc qtybutton btn qty_increase" data-id="${obj[key]['rowId']}">+</div>
                                </div>
                                <button class="cart-remove-btn deleteItem" data-id="${obj[key]['rowId']}">Remove</button>
                            </div>
                            <div class="product-price">
                                <span class="regular-price mr-0">${currencyPrice(obj[key]['weight'] * obj[key]['qty'])}</span>
                                <span class="price">${currencyPrice(obj[key]['price'] * obj[key]['qty'])}</span>
                            </div>
                        </div>
                    </div>
                </div>`;
...
$(document).on('click', '.qty_decrease', function () {
    ...
    $.ajax({
        method: "POST",
        url: $('#CartDecrementFromSession').data("url"), // অবশ্যই route /cart/decrease হওয়া উচিত
        data: {
            _token: $('meta[name="csrf-token"]').attr('content'),
            id: id,
            quantity: quantity2
